...
export class VidssaveDownloader implements MemeDownloader {
  ...
  async steal(url: string): Promise<DownloadResult> {
    ...
    const hardcodedToken = '20250901majwlqo';

    try {
      this.logger.log('Attempting parse with hardcoded token...');
      return await this.executeParse(url, hardcodedToken);
    } catch (hardcodedError) {
      this.logger.warn(
        `Parse with hardcoded token failed: ${hardcodedError.message}. Falling back to blob / parser...`
      );
